...
def calib_fisher_info(model, calib_loader, device, search_layer, use_cache=True): 
    #search_layer = "o_proj"
    model.half()
    model.to(device)
    model_id = model.config._name_or_path
    cache_file = f"/home/mjyun/Palu/cache/{model_id.replace('/','_')}_calib_fisher_info.pt"
    ...
    if os.path.exists(cache_file) and use_cache:
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Linear) and search_layer in name:
...
def rank_search(model: nn.Module, tokenizer, model_id, ratio, search_layer):
    #search_layer = "o_proj"
    #print(model.config) 
    #exit()

    # Prepare Fisher information
    calib_loader = get_calib_data("wikitext2", tokenizer, model_id, 2048, seqlen=1024)
    calib_fisher_info(model, calib_loader, model.device, search_layer)
    ...
    for name, module in model.named_modules():
        if search_layer in name:
            module_rank = model.config.hidden_size
            total_rank += module_rank
            
            select_result.update({name: [4096]*1 })
            fisher = module.fisher_info.reshape(1, -1, module.in_features)
            
            fisher_list = [torch.mean(fisher[i]).item() for i in range(1)]
            fisher_info_dict.update({name: fisher_list})
            fisher_sum += sum(fisher_list)

    target_rank = total_rank * ratio
    ...
    while dif > 0:
        for i in range(len(indexes)):
            if select_result[indexes[i][0]][indexes[i][1]] == 4096:
                continue
            ...
            if dif == 0:
                break
    select_result = split_values(select_result, 32)
